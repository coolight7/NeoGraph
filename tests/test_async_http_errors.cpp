...
TEST(HttpErrorTaxonomy, ClassifyHttpStatus) {
    EXPECT_EQ(async::classify_http_status(500), async::HttpErrorKind::ServerError);
    EXPECT_EQ(async::classify_http_status(503), async::HttpErrorKind::ServerError);
    EXPECT_EQ(async::classify_http_status(429), async::HttpErrorKind::TooManyRequests);
    EXPECT_EQ(async::classify_http_status(413), async::HttpErrorKind::RequestTooLarge);
    EXPECT_EQ(async::classify_http_status(400), async::HttpErrorKind::PayloadInvalid);
    EXPECT_EQ(async::classify_http_status(401), async::HttpErrorKind::PayloadInvalid);

    // Success/redirect don't flow through here.
    EXPECT_EQ(async::classify_http_status(200), async::HttpErrorKind::Unknown);
    EXPECT_EQ(async::classify_http_status(301), async::HttpErrorKind::Unknown);
}
...
TEST(HttpErrorTaxonomy, ClassifyAsioSystemErrors) {
    ...
    // POSIX errno values.
    EXPECT_EQ(
        async::classify_asio_error(asio::error::make_error_code(asio::error::connection_refused)),
        async::HttpErrorKind::ConnectRefused);

    EXPECT_EQ(async::classify_asio_error(asio::error::make_error_code(asio::error::timed_out)),
              async::HttpErrorKind::ConnectTimeout);

    EXPECT_EQ(
        async::classify_asio_error(asio::error::make_error_code(asio::error::connection_reset)),
        async::HttpErrorKind::PeerReset);

    EXPECT_EQ(async::classify_asio_error(asio::error::make_error_code(asio::error::broken_pipe)),
              async::HttpErrorKind::PeerReset);
}
...
TEST(HttpErrorTaxonomy, ClassifyAsioEof) {
    ...
    EXPECT_EQ(async::classify_asio_error(ec), async::HttpErrorKind::PeerEofEarly);
}

TEST(HttpErrorTaxonomy, ClassifyDns) {
    EXPECT_EQ(async::classify_asio_error(asio::error::make_error_code(asio::error::host_not_found)),
              async::HttpErrorKind::DnsPermanent);

    EXPECT_EQ(async::classify_asio_error(
...
TEST(HttpErrorTaxonomy, ClassifySslVerifyFailed) {
    ...
    neograph_asio_error_code ec(ERR_PACK(ERR_LIB_SSL, 0, SSL_R_CERTIFICATE_VERIFY_FAILED),
                                asio::error::get_ssl_category());
    EXPECT_EQ(async::classify_asio_error(ec), async::HttpErrorKind::TlsVerifyFailed);
...
TEST(HttpErrorTaxonomy, ClassifySslShortRead) {
    ...
    auto ec = asio::ssl::error::make_error_code(asio::ssl::error::stream_truncated);
    EXPECT_EQ(async::classify_asio_error(ec), async::HttpErrorKind::PeerEofEarly);
}
...
TEST(HttpErrorTaxonomy, ClassifySslGenericHandshake) {
    ...
    // short-read.
    neograph_asio_error_code ec(ERR_PACK(ERR_LIB_SSL, 0, SSL_R_BAD_PACKET_LENGTH),
                                asio::error::get_ssl_category());
    EXPECT_EQ(async::classify_asio_error(ec), async::HttpErrorKind::TlsHandshakeReset);
...
TEST(HttpErrorTaxonomy, ClassifyUnknownFallsThrough) {
    ...
    EXPECT_EQ(async::classify_asio_error(none), async::HttpErrorKind::Unknown);

    // An errno we don't map (ENOSYS = 38 on Linux) → Unknown.
    neograph_asio_error_code unmapped(ENOSYS, std::system_category());
    EXPECT_EQ(async::classify_asio_error(unmapped), async::HttpErrorKind::Unknown);
}
