        ...
        try {
            for (;;) {
                ...
                const std::string body =
                    request_path == "/x" ? R"({"ok":true})" : R"({"ok":false})";
...
TEST(ConnPool, BasicRoundTrip) {
    ...
    run_on(client_io, [&] {
        return [&]() -> asio::awaitable<void> {
            auto resp = co_await pool.async_post("127.0.0.1", std::to_string(srv.port), "/x",
                                                 R"({"hi":"there"})", {}, false);
            status    = resp.status;
...
TEST(ConnPool, ParallelConverges) {
    ...
    for (int i = 0; i < M; ++i) {
        asio::co_spawn(
            ...
            [&]() -> asio::awaitable<void> {
                for (int r = 0; r < R; ++r) {
                    auto resp = co_await pool.async_post("127.0.0.1", std::to_string(srv.port),
                                                         "/x", "{}", {}, false);
                    EXPECT_EQ(resp.status, 200);
                }
            },
