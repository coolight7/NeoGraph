...
inline HttpErrorKind classify_asio_error(const neograph_asio_error_code& ec) noexcept {
    ...
    if (ec == asio::error::host_not_found_try_again) return HttpErrorKind::DnsTemporary;
...
struct HttpError : std::runtime_error {
    ...
    HttpError(HttpErrorKind k, std::string msg) : std::runtime_error(std::move(msg)), kind(k) {}
};
