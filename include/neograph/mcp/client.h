...
private:
    ...
    MCPTool(std::shared_ptr<detail::StdioSession> session, ToolDefinition definition);
...
public:
    ...
    ListToolsPage list_tools(const std::optional<std::string>& cursor = std::nullopt);
    ...
    asio::awaitable<CallToolResult> call_tool_result_async(const std::string& name,
     ...
     */
    asio::awaitable<json> rpc_call_async(const std::string& method,
