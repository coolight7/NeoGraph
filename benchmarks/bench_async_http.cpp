...
Config parse(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--mode")
            ...
            c.client_threads = std::stoi(next());
        else if (a == "--server-threads")
            c.server_threads = std::stoi(next());
        else if (a == "--port")
            c.port = std::stoi(next());
        else if (a == "--ownership-api")
            c.ownership_api = next();
        else if (a == "--body-bytes")
            c.body_bytes = std::stoi(next());
    }
...
constexpr const char* kRespBody = R"({"content":[{"type":"text","text":"ok"}]})";
        ...
        for (;;) {
            ...
            while (std::getline(is, line)) {
                ...
                if (name == "content-length") {
                    ...
                    if (lv.find("close") != std::string::npos) client_keepalive = false;
                }
            }
...
asio::awaitable<void> acceptor_loop(asio::ip::tcp::acceptor acc, int latency_ms) {
    for (;;) {
        ...
        asio::co_spawn(acc.get_executor(), handle_client(std::move(s), latency_ms), asio::detached);
    }
}
...
struct Server {
    ...
    Server(int port, int threads, int latency_ms) {
        ...
        acc.bind(endpoint);
        acc.listen();
        asio::co_spawn(io, acceptor_loop(std::move(acc), latency_ms), asio::detached);
    ...
    for (int i = 0; i < cfg.concur; ++i) {
        ts.emplace_back([&] {
            ...
            for (int r = 0; r < cfg.rounds; ++r) {
                auto res = cli.Post("/v1/messages", R"({"msg":"ping"})", "application/json");
...
asio::awaitable<void> async_agent(asio::any_io_executor ex, int rounds, int port) {
    const std::string body = R"({"msg":"ping"})";
    for (int r = 0; r < rounds; ++r) {
        try {
            auto resp = co_await neograph::async::async_post(ex, "127.0.0.1", std::to_string(port),
                                                             "/v1/messages", body, {});
            if (resp.status != 200) {
    ...
    for (int i = 0; i < cfg.concur; ++i) {
        asio::co_spawn(
            ...
            [ex = io.get_executor(), rounds = cfg.rounds, port = cfg.port]()
                -> asio::awaitable<void> { co_await async_agent(ex, rounds, port); },
...
asio::awaitable<void> pool_agent(neograph::async::ConnPool& pool, int rounds, int port) {
    const std::string body = R"({"msg":"ping"})";
    for (int r = 0; r < rounds; ++r) {
        try {
            auto resp = co_await pool.async_post("127.0.0.1", std::to_string(port), "/v1/messages",
                                                 body, {}, false);
            if (resp.status != 200) {
...
void run_async_pool(const Config& cfg) {
    ...
    for (int i = 0; i < cfg.concur; ++i) {
        asio::co_spawn(
            io,
            [&pool, rounds = cfg.rounds, port = cfg.port]() -> asio::awaitable<void> {
...
void run_ownership_construct(const Config& cfg) {
    ...
    for (int i = 0; i < total; ++i) {
        if (cfg.ownership_api == "post") {
            auto op = neograph::async::async_post(ex, host, port, path, body, headers, false);
            do_not_optimize(op);
        } else if (cfg.ownership_api == "get") {
            auto op = neograph::async::async_get(ex, host, port, path, headers, false);
            do_not_optimize(op);
        } else if (cfg.ownership_api == "stream") {
            auto op = neograph::async::async_post_stream(ex, host, port, path, body, headers, false,
                                                         on_chunk);
            do_not_optimize(op);
...
int main(int argc, char** argv) {
    ...
    if (cfg.mode == "ownership_construct") {
        ...
        std::printf(
            "mode=%s api=%s ops=%.0f body_bytes=%d wall=%0.6fs "
            "ns/op=%0.1f rss_mb=%0.1f\n",
            cfg.mode.c_str(), cfg.ownership_api.c_str(), total_ops, cfg.body_bytes, wall,
            wall * 1.0e9 / total_ops, peak_rss_mb());
        return 0;
    }
    ...
    const bool driver_is_async = cfg.mode == "async" || cfg.mode == "async_pool";
    double     total_ops       = static_cast<double>(cfg.concur) * cfg.rounds;
    std::printf(
        "mode=%-10s concur=%6d rounds=%d lat_ms=%d  "
        "wall=%6.3fs  ops/s=%9.1f  rss_mb=%7.1f  "
        "client_threads=%d server_threads=%d\n",
        cfg.mode.c_str(), cfg.concur, cfg.rounds, cfg.latency_ms, wall, total_ops / wall,
        peak_rss_mb(), driver_is_async ? cfg.client_threads : cfg.concur, cfg.server_threads);
    return 0;
}
