...
namespace {
json extract_rpc_result(const json& response, int expected_id, std::string_view transport) {
    ...
    if (has_result == has_error) {
        throw std::runtime_error(prefix + " response must contain exactly one of result or error");
    }
    if (has_error) {
    ...
    HttpSession(std::string url, MCPClientConfig client_config)
        ...
        if (prefix.empty() || prefix == "/") {
            path = "/mcp";
                   ...
                   (prefix.size() > 4 && prefix.compare(prefix.size() - 4, 4, "/mcp") == 0)) {
            path = prefix;
        } else {
            path = prefix + "/mcp";
        }
...
asio::awaitable<json> StdioSession::rpc_call_async(const std::string& method, const json& params) {
    ...
    co_return co_await asio::co_spawn(
        ...
        do_exchange(method, params), asio::use_awaitable);
}

asio::awaitable<json> StdioSession::do_exchange(std::string method, json params) {
    const int id = ++next_id_;
    ...
    std::shared_ptr<json> respptr = co_await chan->async_receive(asio::use_awaitable);
...
void validate_schema_value(const json& value, const json& schema, const std::string& path) {
    if (!schema.is_object()) {
        throw std::runtime_error("MCP outputSchema at " + path + " must be an object");
    }
    if (schema.contains("const") && value != schema["const"]) {
        throw std::runtime_error("MCP structuredContent at " + path + " does not match const");
    }
    if (schema.contains("enum") && schema["enum"].is_array()) {
        ...
        if (!matched) {
            throw std::runtime_error("MCP structuredContent at " + path + " is not in enum");
        }
    }
    if (schema.contains("type")) {
        ...
        if (schema["type"].is_string()) {
            ...
            for (const auto& type : schema["type"]) {
                if (type.is_string() && schema_type_matches(value, type.get<std::string>())) {
                    matched = true;
                    break;
                }
            }
        ...
        if (properties.is_object()) {
            for (auto it = properties.begin(); it != properties.end(); ++it) {
                if (value.contains(it.key())) {
                    validate_schema_value(value[it.key()], it.value(), path + "/" + it.key());
                }
            }
        }
        if (schema.value("additionalProperties", true) == false && properties.is_object()) {
    ...
    if (value.is_array() && schema.contains("items") && schema["items"].is_object()) {
        for (std::size_t i = 0; i < value.size(); ++i) {
            validate_schema_value(value[i], schema["items"], path + "/" + std::to_string(i));
        }
    }
}

void validate_tool_result(const CallToolResult& result, const json& output_schema) {
...
MCPTool::MCPTool(std::shared_ptr<detail::StdioSession> session, ToolDefinition definition)
    : stdio_session_(std::move(session)), definition_(std::move(definition)) {}

ChatTool MCPTool::get_definition() const {
    return {definition_.name, definition_.description, definition_.input_schema};
}
...
asio::awaitable<CallToolResult> MCPTool::execute_result_async(const json& arguments) {
    if (stdio_session_) {
    ...
    if (http_session_) {
        auto client = std::unique_ptr<MCPClient>(new MCPClient(http_session_, metadata_));
        auto typed  = co_await client->call_tool_result_async(definition_.name, arguments);
        co_return             typed;
    }
...
MCPClient::MCPClient(const std::string& server_url) : MCPClient(server_url, MCPClientConfig{}) {}

MCPClient::MCPClient(const std::string& server_url, MCPClientConfig config)
    : http_session_(std::make_shared<detail::HttpSession>(server_url, std::move(config))),
...
MCPClient::MCPClient(std::shared_ptr<detail::HttpSession>    session,
                     std::shared_ptr<detail::ClientMetadata> metadata)
    : http_session_(std::move(session)), metadata_(std::move(metadata)) {}
...
asio::awaitable<json> MCPClient::rpc_call_async(const std::string& method, const json& params) {
    ...
    try {
        res = co_await async::async_post(ex, session->endpoint.host, session->endpoint.port,
    ...
    if (res.status != 200) {
        ...
        std::string full_url =
            scheme + session->endpoint.host + ":" + session->endpoint.port + session->path;
        std::string hint;
        if (res.status == 404) {
            hint =
...
void store_initialize_result(const std::shared_ptr<detail::ClientMetadata>& metadata,
...
void reset_initialization(const std::shared_ptr<detail::HttpSession>&    http_session,
...
asio::awaitable<void> send_initialized_notification(
    ...
    auto res     = co_await async::async_post(
        ex, session->endpoint.host, session->endpoint.port, session->path, notify.dump(),
        notification_headers(session), session->endpoint.tls, opts);
...
bool MCPClient::initialize(const std::string& client_name) {
    {
        ...
        while (metadata_->lifecycle == detail::ClientMetadata::Lifecycle::initializing) {
            metadata_->changed.wait(lk);
        }
        if (metadata_->lifecycle == detail::ClientMetadata::Lifecycle::initialized) {
            return true;
        }
...
asio::awaitable<bool> MCPClient::initialize_async(const std::string& client_name) {
    ...
    for (;;) {
        {
            ...
            if (metadata_->lifecycle == detail::ClientMetadata::Lifecycle::created) {
                metadata_->lifecycle = detail::ClientMetadata::Lifecycle::initializing;
                owner                = true;
                break;
            }
    ...
    try {
        auto raw = co_await rpc_call_async("initialize", initialize_params(client_name));
        auto                result = InitializeResult::from_json(raw);
        if (http_session_) {
            {
                std::lock_guard lk(http_session_->mu);
...
bool MCPClient::is_initialized() const noexcept {
    std::lock_guard lk(metadata_->mu);
    return metadata_->lifecycle == detail::ClientMetadata::Lifecycle::initialized;
}
...
std::vector<std::unique_ptr<Tool>> MCPClient::get_tools() {
    ...
    for (auto& definition : definitions) {
        if (stdio_session_) {
            tools.push_back(
                std::unique_ptr<Tool>(new MCPTool(stdio_session_, std::move(definition))));
        } else {
...
std::vector<ToolDefinition> MCPClient::get_tool_definitions() {
    ...
    do {
        ...
        definitions.insert(definitions.end(), std::make_move_iterator(page.tools.begin()),
                           std::make_move_iterator(page.tools.end()));
        if (page.next_cursor == cursor && cursor) {
            throw std::runtime_error("MCP tools/list returned the same nextCursor repeatedly");
        }
...
CallToolResult MCPClient::call_tool_result(const std::string& name, const json& arguments) {
...
asio::awaitable<CallToolResult> MCPClient::call_tool_result_async(const std::string& name,
