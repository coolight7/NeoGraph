...
[[noreturn]] inline void rethrow_first_exception(const asio::multiple_exceptions& error) {
...
inline std::string build_request(std::string_view                                        host,
    ...
    out.append(directive == ConnDirective::keep_alive ? "Connection: keep-alive\r\n"
                                                      : "Connection: close\r\n");
...
inline int parse_status_line(std::string_view line) {
    ...
    auto token =
        (sp2 == std::string_view::npos) ? line.substr(start) : line.substr(start, sp2 - start);
    int status   = 0;
...
inline long extract_headers(std::istream&       in,
    ...
    while (std::getline(in, line)) {
        ...
        if (name == "content-length") {
            ...
            std::transform(value.begin(), value.end(), std::back_inserter(lv),
                           [](unsigned char c) { return std::tolower(c); });
            if (lv.find("close") != std::string::npos) directive = ConnDirective::close;
...
asio::awaitable<StreamExchangeResult> run_exchange_stream(
    Stream& stream, const std::string& req, const std::function<void(std::string_view)>& on_chunk) {
    ...
    if (!chunked) {
        throw std::runtime_error("async_post_stream: response must be Transfer-Encoding: chunked");
    }
    ...
    for (;;) {
        ...
        const std::size_t size_line_len = static_cast<std::size_t>(std::distance(it_start, it_cr));
        ...
        (void)p;
        if (pec != std::errc{}) {
            throw std::runtime_error("async_post_stream: malformed chunk size '" + size_str + "'");
        }
        ...
        if (chunk_size == 0) {
            ...
            if (buf.size() < 2) {
                co_await asio::async_read(stream, buf, asio::transfer_exactly(2 - buf.size()),
                                          asio::use_awaitable);
            }
            auto term    = buf.data();
            ...
            if (*ti != '\r' || ti_next == asio::buffers_end(term) || *ti_next != '\n') {
                throw std::runtime_error("async_post_stream: chunked trailers not supported");
            }
            buf.consume(2);
            break;
        }
        ...
        if (buf.size() < needed) {
            co_await asio::async_read(stream, buf, asio::transfer_exactly(needed - buf.size()),
                                      asio::use_awaitable);
        }
...
asio::awaitable<std::size_t> read_chunk_size(Stream& stream, asio::streambuf& buf) {
    ...
    if (ec != std::errc{}) {
        ...
        throw std::runtime_error("async_post: malformed chunk size '" + size_str + "'");
    }
...
template <typename Stream>
asio::awaitable<void> read_chunked_body(Stream& stream, asio::streambuf& buf, std::string& out) {
    for (;;) {
...
asio::awaitable<ExchangeResult> run_exchange(Stream& stream, const std::string& req) {
    ...
    if (filled < content_length) {
        ...
        co_await asio::async_read(stream, asio::buffer(r.response.body.data() + filled, remaining),
