...
void Agent::ensure_system_message(std::vector<ChatMessage>& messages) {
    if (instructions_.empty()) return;

    bool has_system =
        !messages.empty() && messages[0].role == "system" && messages[0].content == instructions_;
...
ChatCompletion Agent::complete(const std::vector<ChatMessage>& messages) {
