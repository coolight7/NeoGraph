...
SchemaProvider::SchemaProvider(Config config, json schema)
    : user_config_(std::move(config)), schema_(std::move(schema)) {
    parse_schema();
...
std::unique_ptr<SchemaProvider> SchemaProvider::create(const Config& config) {
    json schema;
    ...
    try {
        ...
        throw std::runtime_error("SchemaProvider: invalid JSON in schema file: " +
                                 std::string(e.what()));
    }
...
void SchemaProvider::parse_schema() {
    ...
    conn_.base_url         = user_config_.base_url_override.empty() ? c.value("base_url", "")
    ...
    if (msg.role == "tool") {
        switch (tool_result_.strategy) {
            ...
            case ToolResultStrategy::PARTS_ARRAY: {
                // Gemini: {role:"user", parts:[{functionResponse:{name:"...",
                // response:{result:"..."}}}]}
                std::map<std::string, json> vars;
                vars["ID"]   = msg.tool_call_id;
                vars["NAME"] = msg.tool_name;
...
ChatMessage SchemaProvider::parse_response(const json& resp_json) const {
    ...
    switch (resp_.strategy) {
        case ResponseStrategy::CHOICES_MESSAGE: {
            ...
            if (!message) {
                throw std::runtime_error("SchemaProvider: cannot find message at path: " +
                                         resp_.message_path);
            }

            msg.role = message->value(resp_.role_field, "assistant");

            if (message->contains(resp_.content_field) &&
                !(*message)[resp_.content_field].is_null()) {
...
std::string SchemaProvider::parse_stop_reason(const json& resp_json) const {
    auto map_reason = [](const json& value, const std::map<std::string, std::string>& mapping,
    ...
    if (!resp_.stop_reason_path.empty()) {
        if (const auto value = json_path::at_path(resp_json, resp_.stop_reason_path)) {
            if (const auto mapped = map_reason(*value, resp_.stop_reason_map, true);
                !mapped.empty()) {
                return mapped;
            }
        }
...
asio::awaitable<ChatCompletion> SchemaProvider::complete_async(const CompletionParams& params) {
    ...
    if (curl_pool_) {
        std::string url =
            ...
            (endpoint.port == "443" || endpoint.port == "80" ? "" : ":" + endpoint.port) +
            endpoint_path;
        request.emplace(
            curl_pool_->async_post(std::move(url), std::move(body_str), std::move(headers), opts));
    } else {
        request.emplace(conn_pool_->async_post(endpoint.host, endpoint.port, endpoint_path,
                                               std::move(body_str), std::move(headers),
                                               endpoint.tls, opts));
    }
    ...
    if (user_config_.use_websocket) {
        if (provider_name_ != "openai-responses") {
            throw std::runtime_error(
                ...
                "the openai-responses schema (got: " +
                provider_name_ + ")");
        }
...
ChatCompletion SchemaProvider::complete_stream_http(
                    ...
                    try {
                        ...
                        if (stream_.delta_strategy == "candidates_parts") {
                            ...
                            if (parts && parts->is_array()) {
                                for (const auto& part : *parts) {
                                    if (part.contains(stream_.delta_text_field) &&
                                        ...
                                        std::string token =
                                            part[stream_.delta_text_field].get<std::string>();
                                        full_content += token;
                                    ...
                                    if (part.contains(stream_.delta_function_call_field)) {
                                        ...
                                        call.name =
                                            fc.value(stream_.delta_tool_call_name_field, "");
                                        if (fc.contains(stream_.delta_tool_call_args_field)) {
                                            call.arguments =
                                                fc[stream_.delta_tool_call_args_field].dump();
                                        }
                            ...
                            if (delta->contains(stream_.content_field) &&
                                ...
                                std::string token =
                                    (*delta)[stream_.content_field].get<std::string>();
                                full_content += token;
                                if (on_chunk) on_chunk(token);
                            }
                            ...
                            if (delta->contains(stream_.tool_calls_field)) {
                                for (const auto& tc : (*delta)[stream_.tool_calls_field]) {
                                    ...
                                    if (tc.contains(stream_.tool_call_id_field)) {
                                        tc_map[idx].id =
                                            tc[stream_.tool_call_id_field].get<std::string>();
                                    }
                                    auto name_node =
                                        json_path::at_path(tc, stream_.tool_call_name_path);
                                    if (name_node && name_node->is_string()) {
                                        tc_map[idx].name += name_node->get<std::string>();
                                    }
                                    auto args_node =
                                        json_path::at_path(tc, stream_.tool_call_args_path);
                ...
                // --- SSE_EVENTS format (Claude) ---
                    ...
                    try {
                        ...
                        if (action == "ignore") {
                            ...
                            // Claude default: block_path="content_block", tool_type="tool_use", id
                            // in "id". Responses:      block_path="item",
                            // tool_type="function_call", id in "call_id".
                            ...
                            std::string text_delta_type =
                                event_cfg.value("text_delta_type", "text_delta");
                            std::string tool_delta_type =
                                event_cfg.value("tool_delta_type", "input_json_delta");
                            ...
                            if (delta_type == text_delta_type) {
                                ...
                                std::string args_fld =
                                    event_cfg.value("tool_args_field", "partial_json");
                            ...
                            if (j.contains("response") && j["response"].is_object()) {
                                completion.usage = parse_usage(j["response"]);
                            }
                        }
                    } catch (...) {
                        // Skip malformed
    ...
    if (res->status != 200) {
        if (res->status == 429) {
            throw RateLimitError("API error (HTTP 429): " + res->body, retry_after_seconds(res));
        }
        throw std::runtime_error("API error (HTTP " + std::to_string(res->status) +
                                 "): " + res->body);
    }
...
asio::awaitable<ChatCompletion> SchemaProvider::complete_stream_async(
    const CompletionParams& params, const StreamCallback& on_chunk) {
    ...
    if (user_config_.use_websocket && provider_name_ == "openai-responses") {
        ...
        co_return co_await asio::co_spawn(
            exec, complete_stream_ws_responses(operation_params, on_chunk),
            asio::bind_cancellation_slot(operation->slot(), asio::use_awaitable));
    }
    ...
    if (operation) {
        ...
        operation_params.cancel_token = operation;
        operation->slot().assign([cancel_control](asio::cancellation_type_t type) {
            if (!type) return;
    ...
    asio::dispatch(bridge_exec, [this, params = std::move(operation_params), wrapped, shared,
                                 cancel_control]() mutable {
        std::optional<ChatCompletion> result;
        std::exception_ptr            err;
        try {
            result = this->complete_stream_http(params, wrapped, cancel_control);
        } catch (...) {
...
asio::awaitable<ChatCompletion> SchemaProvider::complete_stream_ws_responses(
    const CompletionParams& params, const StreamCallback& on_chunk) {
    ...
    auto               ws =
        co_await       async::ws_connect(ex, endpoint.host, endpoint.port,
                                   endpoint.prefix.empty() ? std::string("/v1/responses")
                                                                 : (endpoint.prefix + "/v1/responses"),
                                         std::move(ws_headers), endpoint.tls);
    ...
    while (!got_done) {
        ...
        if (ws_debug) {
            std::cerr << "[WS DEBUG] recv op=" << static_cast<int>(msg.op)
                      << " bytes=" << msg.payload.size() << " payload=" << msg.payload << "\n";
        }
        if (msg.op == async::WsOpcode::Close) {
            ...
            if (msg.payload.size() >= 2) {
                std::uint16_t code = (static_cast<std::uint8_t>(msg.payload[0]) << 8) |
        ...
        if (event_type == "error") {
            ...
            throw std::runtime_error("openai-responses ws error: " + err_msg);
        }
        ...
        if (action == "ignore") {
            ...
            if (event_block_index < 0 || event_block_index >= static_cast<int>(event_blocks.size()))
                continue;
